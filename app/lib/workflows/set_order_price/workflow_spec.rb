describe SoT::SetOrderPrice::Workflow do
  ...
  it 'sets price of an order' do
    expect {
      subject.call(order_id: order.id, requester: user, price: 100, currency: 'usd')
    }.to change {
  ...
  it 'returns successful result if all is good' do
    result = subject.call(order_id: order.id, requester: user, price: 100, currency: 'usd')
    expect(result.order).to eq order
    expect(result.success?).to eq true
  end

  it 'returns unsuccessful result if there are some errors' do
    result = subject.call(order_id: '404', requester: user, price: 100, currency: 'usd')
